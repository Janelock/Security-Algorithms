...
namespace SecurityLibrary
{
    public class PlayFair : ICryptographicTechnique<string, string>
    {
        ...
        public string Analyse(string cipherText)
        {
            ...
            Dictionary<string, int> pairMap = new Dictionary<string, int>();

            for (int i = 0; i < cipherText.Length - 1; i+=2)
            {
                string pair = cipherText.Substring(i, 2);
                if (pair.Trim().Length == 2)
                {
            ...
            var orderedPairs = pairMap.OrderByDescending(pair => pair.Value);
            List<string> strings = new List<string>() {
                "TH","HE", "AN", "IN", "ER", "ON", "RE", "ED", "ND", "HA", "AT", "EN", "ES", "OF", "NT",
